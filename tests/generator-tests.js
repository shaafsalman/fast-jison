...
suite.test('Generator creation', () => {
  ...
  generatorTypes.forEach(type => {
    const options = { type };
    try {
      const generator = new Jison.Generator(grammar, options);
      assert(generator, `${type} generator should be created`);
      
      // Verify productions have IDs
      assert(generator.productions.every(p => typeof p.id !== 'undefined'), 'All productions should have IDs');
      assert(generator.type === type || 
            ...
            (type === 'll' && generator.type === 'LL(1)'),`Generator type should match requested type (${type}, got ${generator.type})`);
    } catch (error) {
        assert.fail(`Failed to create ${type} generator: ${error.message}`);
      }
  });
});
